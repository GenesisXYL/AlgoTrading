...
class Vwap:
    ...
    def vwap_target(self, bar_num, coefs):
    ...
    def record_trade(self, trade_df, idx, trade_px, trade_qty, current_bar, trade_type):
    ...
    def calc_order_quantity(self, raw_order_qty, round_lot, qty_remaining):
    ...
    def calc_schedule_factor(self, max_behind, target_shares, quantity_filled, order_quantity):
        ...
        schedule_factor = (self, quantity_behind / max_behind)
    ...
    # MAIN ALGO LOOP
    def algo_loop(self, trading_day, order_side, original_order_quantity, vwap_coefficients):
        ...
        order_targets = self.vwap_target(np.arange(0, 390, dtype='int64'), vwap_coefficients) * original_order_quantity
        ...
        for index, row in trading_day.iterrows():
            ...
            if pd.isna(row.trade_px):  # it's a quote
                ...
                if live_order:
                    if (order_side == 'b') and (last_price <= live_order_price):
                        ...
                        self.self.record_trade(trades, index, live_order_price, fill_size, current_bar, 'p')
                    ...
                    if (order_side == 's') and (last_price >= live_order_price):
                        fill_size = min(live_order_quantity, last_size)
                        self.record_trade(trades, index, live_order_price, fill_size, current_bar, 'p')
                        total_quantity_filled += fill_size
            ...
            schedule_factor = self.calc_schedule_factor(max_behind, current_target_shares,
            ...
            if order_side == 'b':
                if (fair_value >= ask_price or current_bar == last_minutes_from_open) and quantity_behind > round_lot:
                    ...
                    new_order_quantity = self.calc_order_quantity(quantity_behind, round_lot, quantity_remaining)
    
                    self.record_trade(trades, index, new_trade_price, new_order_quantity, current_bar, 'a')
                    ...
                    if quantity_behind > round_lot:
                        ...
                        live_order_quantity = self.calc_order_quantity(quantity_behind, round_lot, quantity_remaining)
