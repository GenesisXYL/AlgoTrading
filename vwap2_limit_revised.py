...
def calc_schedule_factor(max_behind, target_shares, quantity_filled, order_quantity):
    ...
    #print(quantity_behind, max_behind)
...
# MAIN ALGO LOOP
def algo_loop(trading_day, order_side, original_order_quantity, vwap_coefficients, schedule_coef, tick_coef=0, limit_price = 0):
    log_message('Beginning VWAP run: {:s} {:d} shares'.format(order_side, original_order_quantity))
    log_message('schedule_coef: {}, tick_coef: {}'.format(schedule_coef, tick_coef))
    ...
    for index, row in trading_day.iterrows():
        ...
        #print(fair_value, midpoint, ask_price, bid_price, schedule_coef, schedule_factor)
        ...
        if order_side == 'b':
                ...
                if index < last_index:
                    if fair_value >= ask_price and quantity_behind > round_lot:
                        if limit_price > ask_price:
                            ...
                            live_order_price = 0.0
                            live_order = False
                        else: #    limit price < ask, no trade now
                            live_order_price = limit_price
                        ...
                        if quantity_behind > round_lot:
                            live_order_price = min(bid_price, limit_price)
                            live_order_quantity = calc_order_quantity(quantity_behind, round_lot, quantity_remaining)
                            live_order = True

                # Ensure that your algo trades the exact number of shares in the order
                else: #index == last_index
                    if quantity_remaining > 0:
                        total_agg_count += 1
                        new_trade_price = ask_price
                        new_order_quantity = quantity_remaining
                        quantity_remaining = max(0, quantity_remaining - new_order_quantity)
                        record_trade( trades, index, new_trade_price, new_order_quantity, current_bar, 'a' )
            else: # limit_price = 0, no limit
                if index < last_index:
                    if fair_value >= ask_price and quantity_behind > round_lot:
                        # trade happened
                        total_agg_count += 1
            ...
            if limit_price > 0:
                if index < last_index:
                    if fair_value <= bid_price and quantity_behind > round_lot:
                        if limit_price < bid_price:
                            ...
                            live_order_price = 0.0
                            live_order = False
                        else: #    limit price > bid_price, no trade now
                            live_order_price = limit_price
                            live_order_quantity = calc_order_quantity(quantity_behind, round_lot, quantity_remaining)
                            # live_order_quantity = quantity_behind
                            live_order = True
                        ...
                        if quantity_behind > round_lot:
                            live_order_price = max(ask_price, limit_price)
                            live_order_quantity = calc_order_quantity(quantity_behind, round_lot, quantity_remaining)
                            live_order = True
                else: # last bar, want to clear
                    if quantity_remaining > 0:
                        total_agg_count += 1
                        new_trade_price = bid_price
                        new_order_quantity = quantity_remaining
                ...
                if index < last_index:
                    if fair_value <= bid_price and quantity_behind > round_lot:
                        # aggresive sell, because the fair value is below the bid
                        total_agg_count += 1
    ...
    log_message('Quantity Remaining: {}'.format(quantity_remaining))
