...
# MAIN ALGO LOOP
def algo_loop(trading_day, order_side, original_order_quantity, vwap_coefficients, schedule_coef, tick_coef = 1., limit_price):
        ...
        if order_side == 'b':
            if (fair_value >= ask_price or current_bar == last_minutes_from_open) and quantity_behind > round_lot:
                ...
                if limit_price < bid_price :
                    ...
                    live_order_price = 0.0
                    live_order = False
                else: #    bid < limit price < ask, no trade now
                    live_order_price = limit_price
                    live_order_quantity = calc_order_quantity(quantity_behind, round_lot, quantity_remaining)
